...
import numpy as np
import pickle
from labs_model1 import run_labs,lab_name_dict, calculate_statistics, convert_unit
...
# biomarker_list = ['KRAS','TP53','TMB','PD-L1','EGFR','BRAF','PTEN','TERT','KIT','PIK3CA','CDH1','STK11','ALK']
# biomarker_list = ['KRAS','TP53','EGFR','ALK','BRAF','TMB','ROS1','MET','ERBB2','RET','PD-L1']
biomarker_list = ['KRAS','TP53','EGFR','ALK','BRAF','TMB','ROS1','MET','ERBB2','RET','PD-L1','PTEN','TERT','KIT','PIK3CA','CDH1','STK11','NTRK1','NTRK2','NTRK3','FGFR3','FGFR4'] #MSI
...
,'dacomitinib','amivantamab','mobocertinib','necitumumab','adagrasib','crizotinib','ceritinib','alectinib','brigatinib','lorlatinib']))
    ...
    med = med.merge(cond_table[['chai_patient_id','line_start','random_date']],on='chai_patient_id',how='inner')
    ...
    med_subset = med[['chai_patient_id', 'line_start','med_generic_name_standard_name']].drop_duplicates()
    ...
    med_df = pd.DataFrame(columns=['chai_patient_id','line_start'] + med_list)
    med_df[['chai_patient_id','line_start']] = cond_table[['chai_patient_id','line_start']]
    for i,j in med_df.iterrows():
        pat_df = med_subset[(med_subset['chai_patient_id']==j['chai_patient_id'])&(med_subset['line_start']==j['line_start'])]
...
#     med_code_list = conmed_value_df.code.unique()
    conmed = conmed.merge(cond_table[['chai_patient_id','line_start','random_date']],on='chai_patient_id',how='inner')
...
#     conmed_subset = conmed
    ...
    conmed_subset = conmed[['chai_patient_id', 'line_start',
    ...
    conmed_list = list(conmed_subset.med_generic_name_standard_name.unique())
    conmed_df = pd.DataFrame(columns=['chai_patient_id','line_start'] + conmed_list)
    conmed_df[['chai_patient_id','line_start']] = cond_table[['chai_patient_id','line_start']]
    for i,j in conmed_df.iterrows():
        pat_df = conmed_subset[(conmed_subset['chai_patient_id']==j['chai_patient_id'])&(conmed_subset['line_start']==j['line_start'])]
        med_list_pat = list(pat_df.med_generic_name_standard_name.unique())
        conmed_df.loc[(conmed_df['chai_patient_id']==j['chai_patient_id'])&(conmed_df['line_start']==j['line_start']),med_list_pat]=1
...
#     lot_all_dates_merged = lot_all_dates_merged[lot_all_dates_merged.apply(lambda x:x['stage_date']>=x['line_start'],axis=1)]
    ...
    stage_df = cond_table_merged_stage[['chai_patient_id','line_start','stage_date', 'stage', 'tstage', 'mstage', 'nstage']]
    stage_df['stage'] = stage_df.groupby(['chai_patient_id','line_start'])['stage'].apply(lambda x: x.fillna(x.max()))
    stage_df['tstage'] = stage_df.groupby(['chai_patient_id','line_start'])['tstage'].apply(lambda x: x.fillna(x.max()))
    stage_df['nstage'] = stage_df.groupby(['chai_patient_id','line_start'])['nstage'].apply(lambda x: x.fillna(x.max()))
    stage_df['mstage'] = stage_df.groupby(['chai_patient_id','line_start'])['mstage'].apply(lambda x: x.fillna(x.max()))
    stage_df = stage_df.drop_duplicates(subset = ['chai_patient_id', 'line_start'])
    return stage_df
...
def get_biomarker(gn_schema,pt_schema,cond_table,cohort):
    ...
    Crosswalk = dtu.redshift_to_df('''SELECT * FROM {schema}.master_chai_id_map'''.format(schema='cdm_structured_202402'))
...
biomarker_variant_type_standard_name,test_result_standard_name,report_date FROM {schema}.biomarker'''.format(schema=pt_schema))
    ...
    cond_table = cond_table.drop_duplicates(subset=['chai_patient_id','line_start'],keep='first')
    return cond_table[['chai_patient_id','tp53_mutation','line_start']]

    
## Considers multiple biomarker
def get_all_biomarker(gn_schema,pt_schema,cond_table,cohort):
    non_curation_pat_list = list(cond_table[cond_table['curation']==0]['chai_patient_id'].unique())
...
#                             (biomarker_df_pt360['biomarker_variant_type_standard_name'].isin(['gene mutation', 'small indel']))]
  ...
  'Unknown (qualifier value)','veristrat good','veristrat poor']:
    ...
    biomarker_feat_df = cond_table[['chai_patient_id','line_start']]
    for biomarker_ele in biomarker_list:
        ...
        if temp_df.shape[0]>0:
            ...
            temp_df = temp_df.merge(cond_table[['chai_patient_id','random_date','line_start']],on='chai_patient_id',how='inner')
            ...
            temp_df = temp_df.drop_duplicates(subset=['chai_patient_id','line_start'],keep='first')
            biomarker_feat_df = biomarker_feat_df.merge(temp_df[['chai_patient_id','line_start',f'biomarker_{biomarker_ele}']],on=['chai_patient_id','line_start'],how='left')
        else:
